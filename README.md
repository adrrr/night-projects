...
Jeu de précision ultra-minimaliste — l'exact opposé visuel de Dead Stop : encre noire sur papier près-blanc, zéro neon, zéro particule, un seul chiffre. Une fine aiguille (1px) traverse toute la page et tourne à vitesse constante autour du centre ; une ligne d'horizon discrète la croise à l'horizontale. Tu n'as qu'UN tap, qu'UNE chance : fige l'aiguille PILE sur la ligne. Personne ne peut être parfaitement dessus — tout est de savoir qui s'en rapproche le plus, mesuré en degrés au dix-millième près. Quand l'aiguille est parfaitement à plat, les deux traits fusionnent en un seul trait pleine largeur. UN seul jeu : tout le monde reçoit la même aiguille seedée à partir de la date UTC du jour (le défi se réinitialise à minuit UTC). Tape n'importe où pour démarrer — aucun choix avant de jouer. Un score compte pour DEUX classements à la fois : **Daily** (top 10 du jour) et **Global** (top 10 all-time), via un seul Cloudflare Worker + KV. Un score qualifié (top 10 sur l'un OU l'autre) → tu inscris tes initiales façon arcade ; sur l'écran du classement, un toggle Daily/Global bascule l'affichage entre les deux tableaux, ta ligne surlignée. Hairlines crispes gérées au devicePixelRatio, mouvement en delta-time (identique à 60 et 120 Hz), Web Audio discret + mute, record perso en localStorage, rate-limit anti-flood côté serveur. Mobile-first (tap) + desktop (clic/espace). Trivial à apprendre, impossible à maîtriser.
